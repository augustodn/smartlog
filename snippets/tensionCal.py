...
class TensionCal(QDialog, Ui_TensionCal):
    ...
    def check_cal(self):
        ...
        deltanTools = (nToolsRead - nToolsCalc) / (nToolsCalc * 1.0)
        deltatStr = (tStrRead - tStrCalc) / (tStrCalc * 1.0)

        if abs(deltanTools) < 0.05 and abs(deltatStr) < 0.05:
    ...
    def read_data(self):
        ...
        if data and self.coef:
            if not self.noToolsCal:
                avg = self.calc_average(data, col)
